      ...
      <div className="hidden lg:flex w-1/2 bg-gradient-to-br from-blue-700 to-purple-800 p-25 text-white flex-col justify-center relative overflow-hidden">
        ...
        <div className="relative z-10 gap-2 flex flex-col items-start">
          <div className="flex items-center gap-2 font-bold text-2xl mb-5 cursor-pointer" onClick={() => navigate('/')}>
            <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-blue-700 shadow-lg"><BsStars size={24} /></div>
            ITCareerMatch
          </div>
          ...
          <h1 className="text-3xl font-extrabold leading-tight mb-5">Buka Potensi Penuh <br /> CV Anda Sekarang!</h1>
          <p className="text-blue-100 text-lg max-w-md leading-relaxed mb-5">Bergabung dengan 10,000+ pencari kerja Indonesia yang sudah mendapatkan pekerjaan impian mereka.</p>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 bg-slate-50 flex-col overflow-y-auto">
        ...
        <div className="w-full max-w-md bg-white rounded-[2rem] p-8 shadow-xl shadow-slate-200/60">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" required value={name} onChange={(e) => setName(e.target.value)} placeholder="Contoh: Budi Santoso" className="w-full pl-11 pr-4 py-3.5 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none text-sm bg-slate-50 focus:bg-white transition-all"/>
              </div>
            </div>
          ...
          <p className="text-center text-sm text-slate-500 mt-6">
            Sudah punya akun?{' '}
            <Link to={`/login${redirectTarget !== '/dashboard' ? `?redirect=${redirectTarget}` : ''}`} className="font-semibold text-blue-600 hover:underline cursor-pointer">Masuk</Link>
          </p>
        </div>

        <p className="text-center text-xs text-slate-400 mt-6">Dengan mendaftar, Anda menyetujui Ketentuan Layanan dan Kebijakan Privasi kami.</p>
      </div>
    </div>
  );
}